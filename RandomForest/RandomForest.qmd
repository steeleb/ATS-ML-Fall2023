...
-   What's the best way (computationally, and code-wise) to perform these time series
trainings?
 -- ya gotta hack it. either (a) use an enesemble of the models, or re-train on all the data and use the resulting model
-   how do I establish a baseline for this? better than yesterday-is-today? (when I don't have a daily timeseries, can I do this?)
 -- maybe try a global daily average of the last 10y or something? Hopefully this will create a near-daily TS and can go from there.
-   is this the best way to use static variables? i.e. is there some other fancy way of doing it?
...
No pre-processing was completed for these data for regularization, as decision trees make purely 
...
- leave one out by lake -- look at Willard et al for their cv method
-- if the cv's are all performing reasonably, combine all data, model and use to forecast
-- if the cv's perform differently, consider an ensemble method
...
mae_errors = abs(predictions2 - labels_2)
...
mse_errors = np.sqrt(np.mean((predictions2 - labels_2)**2))
...
# Use the forest's predict method on the test data
predictions3 = rf2.predict(features3)

# Use testing set to validate the performance
# Print out the mean absolute error (MAE)
mae_errors2 = abs(predictions3 - labels_3)
#print('Baseline error (MAE): ', round(np.mean(mae_baseline_errors), 2))
print('Error (MAE): ', round(np.mean(mae_errors2), 2))
mae_err_text2 = round(np.mean(mae_errors2), 2)

# See its performance (mean squared errors)
mse_errors2 = np.sqrt(np.mean((predictions3 - labels_3)**2))
#print('Baseline error (MSE): ', round( mse_baseline_errors, 2))
print('Error (MSE): ', round(mse_errors2, 2))
mse_err_text2 = round(mse_errors2, 2)
```
...
# Use the forest's predict method on the test data
predictions4 = rf3.predict(features4)

# Use testing set to validate the performance
# Print out the mean absolute error (MAE)
mae_errors3 = abs(predictions4 - labels_4)
#print('Baseline error (MAE): ', round(np.mean(mae_baseline_errors), 2))
print('Error (MAE): ', round(np.mean(mae_errors3), 2))
mae_err_text3 = round(np.mean(mae_errors3), 2)

# See its performance (mean squared errors)
mse_errors3 = np.sqrt(np.mean((predictions4 - labels_4)**2))
#print('Baseline error (MSE): ', round( mse_baseline_errors, 2))
print('Error (MSE): ', round(mse_errors3, 2))
mse_err_text3 = round(mse_errors3, 2)
```

## Results

Algorithms trained on early data performed better than those trained on larger portions
of the dataset. The first train-val window resulted in a MAE of `r py$mae_err_text` and MSE of `r py$mse_err_text`, where the last window (including all earlier data) resulted in a MAE of `r py$mae_err_text3` and MSE of `r py$mse_err_text3`.
...
pred = py$predictions2
train_val2$pred = pred

ggplot(train_val2, aes(x = date, y = value)) +
  facet_grid(feature ~ .) +
  geom_point() +
...
ggplot(train_val2, aes(x = value, y = pred ,color = feature)) +
  geom_point() +
  theme_bw() +
